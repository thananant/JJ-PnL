...
- **แดชบอร์ดต้องล็อกอิน** ด้วยบัญชีกลาง `pnl_users` (ชุดเดียวกับ P&L / หน้าศูนย์รวมแอพ)
  · สูตร hash `sha256hex(user+'|'+pass+'|JJPNL')` แชร์ทั้ง repo — **ห้ามเปลี่ยน**
- **ใบผ่านกลาง = localStorage `jjpnl_auth`** `{u, h: pass_hash, t: เวลาใช้งานล่าสุด}` ใช้ร่วมกับ P&L/JJ Access
  · ล็อกอินที่หน้าศูนย์รวมแอพแล้ว → เปิด KPI ต่อได้เลยไม่ต้องกรอกซ้ำ (hub เรียก `spreadSession`)
  · เปิด URL ตรง ๆ โดยไม่มีใบผ่าน / ใบผ่านทิ้งไว้เกิน **30 นาที** (`AUTH_IDLE_MS` เท่ากับ P&L) → ต้องใส่รหัสใหม่
  · ทุกครั้งที่ผ่านประตูจะ**เช็คกับฐานข้อมูลจริง** (active + pass_hash ตรง) — ปิดบัญชี/เปลี่ยนรหัสแล้วใบผ่านเดิมใช้ไม่ได้ทันที
  · เช็คไม่ได้เพราะเน็ต/ฐานข้อมูลล่ม **แต่ใบผ่านยังไม่หมดอายุ → ให้ใช้งานต่อ** (อย่าตัดออก — กันล็อกคนทำงาน)
  · ออกจากระบบ = ล้างใบผ่านทุกแอพ (`SESS_KEYS`) เหมือนปุ่มออกที่หน้าแรก
- **สิทธิ์รายแอพ**: `canUseKpi()` ใช้กติกาเดียวกับ `canOpen()` ของ hub — admin/owner ผ่านเสมอ · `apps` ว่าง (ยังไม่เคยตั้งสิทธิ์) = ไม่ล็อกใคร · นอกนั้นต้องมี `apps.kpi.<หน้าจอ>` ที่มี `v`
  · ไม่มีสิทธิ์ → หน้า "ยังไม่ได้รับสิทธิ์" + ปุ่มเข้าด้วยบัญชีอื่น (ไม่เห็นข้อมูลลูกค้าเลย)
- **`?kiosk=` ไม่ต้องล็อกอิน** — เป็นจอให้ลูกค้าหน้าร้านกด (ถ้าใส่ประตูตรงนี้ แท็บเล็ตจะใช้ไม่ได้)
- KPI ถูกเอาออกจาก `ALWAYS` ใน `index.html` แล้ว (คุมสิทธิ์ได้เองจึงซ่อนไอคอนจากคนไม่มีสิทธิ์ได้)
...
- harness ป้อนใบผ่านกลางให้อัตโนมัติ (เหมือนล็อกอินจากหน้าแรกมาแล้ว) — ตัวเลือกใน `boot(url, db, extra)`:
  `noAuth:true` = ไม่มีใบผ่าน (เปิด URL ตรง) · `as:'manager'` = ล็อกอินเป็นคนอื่น · `authAge:ms` = ใบผ่านเก่า
