...
- แอปที่ยังใช้บัญชีคนละชุด (นับสต๊อก `sc_users` · ครัวกลาง `app_users`/Supabase Auth) และ KPI (ไม่มีล็อกอิน) อยู่ในรายการ `ALWAYS` = **แสดงเสมอ** ล็อกอินกลางยังคุมไม่ถึง ต้องล็อกอินในแอปนั้นเอง
- **ซ่อมบำรุง (`jjmk-maint.html`) ต่อล็อกอินกลางแล้ว (2026-09-21)** — ไม่อยู่ใน `ALWAYS` แล้ว คุมด้วยสิทธิ์ `apps.maint` ได้จริง · ใช้กลไก **ตั๋ว** แทนการแจกใบผ่านถาวร:
  - กดไอคอนจากหน้าศูนย์รวมแอป → `handTicket()` ฝาก `jjhub_ticket` (มี `app`/`u`/`role`/`apps`, **อายุ 3 นาที**) แอปปลายทางกินตั๋วแล้วลบทิ้ง เข้าได้เลยตามสิทธิ์
  - **เปิด URL ของแอปตรง ๆ = ต้องกรอกรหัสใหม่เสมอ** (เจ้าของสั่ง: กลัวคนได้ลิงก์ไปแล้วเข้าได้) — เซสชันเก็บใน `sessionStorage` (`jjmt_sess`) รีเฟรช/ลากลงยังอยู่ แต่ปิดแท็บแล้วต้องล็อกอินใหม่
  - ติ๊ก "จำไว้ในเครื่องนี้ 12 ชม." → `jjmt_keep` (เก็บ username+pass_hash, เช็คกับ `pnl_users` ใหม่ทุกครั้งที่เปิด)
  - หน้าล็อกอินของแอปรับได้ทั้ง **`pnl_users`** (บัญชีกลาง — ลองก่อน) และ **`app_users`** (บัญชีเดิมของนับสต๊อก/ครัวกลาง — ลองต่อถ้าไม่เจอในบัญชีกลาง)
  ...
  - หน้าจอในระบบสิทธิ์ (JJ Access): `sched` ตารางบำรุงรักษา · `log` บันทึกงาน+รูปถ่าย · `asset` นับอุปกรณ์+สถิติ · `tasks` ตั้งค่างาน/อุปกรณ์ · **เพิ่มแอปใหม่ที่ล็อกอินเอง ให้ใช้กลไกตั๋วนี้ซ้ำได้เลย**
