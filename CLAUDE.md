# JJ-PnL — จริงใจหมูกระทะ (บริษัท พากันรวย ฟู้ดส์ คอร์ปอเรชั่น จำกัด)

รวมแอปหลังบ้านของร้านหมูกระทะ 4 สาขา (JJLP ลาดพร้าว · JJRD รัชดา · JJCK ครัวกลาง · OFFICE)
ทุกแอปเป็น HTML ไฟล์เดียว รันจาก branch `main` ผ่าน GitHub Pages: https://thananant.github.io/JJ-PnL/
รายชื่อแอปทั้งหมดดู `README.md` · ผู้ใช้สื่อสารภาษาไทย — UI/คำตอบเป็นไทย

## กติกา repo

- **ห้ามเพิ่มไฟล์ `.sql` ลง main หรือ branch งาน** — SQL ทั้งหมดอยู่ branch `sql` เท่านั้น (ไม่รวมเข้า main และไม่ merge กับใคร) · SQL ของระบบเงินเดือนอยู่ในโฟลเดอร์ `jjmk-payroll/` ของ branch นั้น · SQL ระบบ KPI คือ `jjmk-kpi.sql` ที่ root ของ branch นั้น
- กติกาเจ้าของ (2026-08-04): แก้เสร็จ+ตรวจผ่านแล้ว → เปิด PR + merge เข้า main อัตโนมัติ ไม่ต้องถามยืนยัน (ยกเว้นงานที่เสี่ยงลบ/แก้ข้อมูลจริงใน Supabase — ถามก่อน)
- กติกาเจ้าของ (2026-09-17 — แทนที่กติกาก่อนหน้าทั้งหมดเรื่องวิธีส่ง SQL): SQL ที่ต้องรันใน Supabase → **แปะเนื้อหาลงในแชทเป็นบล็อกโค้ด** ให้เจ้าของกดคัดลอกไปวางใน SQL Editor ได้ทันที · **ห้ามส่งเป็นการ์ดไฟล์ (`SendUserFile`) เด็ดขาด** — กดแล้วเด้งหน้าต่างเซฟลงเครื่อง เจ้าของไม่ต้องการดาวน์โหลด · เขียนไฟล์ลง `sql/` ไว้ด้วย (อยู่ใน `.gitignore`) แล้วบอก path ท้ายข้อความเผื่ออยากเปิดในแผงขวา — ยัง push เก็บที่ branch `sql` ตามกติกาข้อแรกทุกครั้ง · งานที่ขึ้น GitHub ให้ push/merge เองเลย ไม่ต้องส่งไฟล์มาในแชท

## ระบบวัดความพึงพอใจลูกค้า (JJ KPI)

- แอปคือ `jjmk-kpi.html` ที่ root (ย้ายมาจาก repo `thananant/JJ-KPI` เมื่อ 2026-09-06) — **อ่าน `kpi/CLAUDE.md` ก่อนแก้ทุกครั้ง**
- ลูกค้ากดหน้าร้านผ่านแท็บเล็ต (`?kiosk=JJRD` / `?kiosk=JJLP`) · ไม่มี param = แดชบอร์ดเจ้าของ
- ตาราง Supabase ใช้ prefix `kpi_` ทั้งหมด (โปรเจกต์เดียวกับระบบอื่น) — **ห้ามแตะตารางที่ไม่ใช่ `kpi_*`**
- รายชื่อพนักงานให้ลูกค้าชม ซิงก์อัตโนมัติจากตาราง `employees` ของ payroll + โชว์เฉพาะคนกำลังเข้างานตามสแกน `punches` (RPC `kpi_sync_staff` / `kpi_on_duty` — อ่านอย่างเดียว)
- **จอลูกค้าโชว์เฉพาะคนที่สแกนเข้างานอยู่จริงเท่านั้น (เจ้าของสั่ง 2026-09-21)** — เช็คไม่ได้ว่าใครอยู่ (ยังไม่รัน SQL/RPC ล้ม/ไม่ได้ผูก payroll) = **ข้ามคำถามชมพนักงาน** ห้ามโชว์ทั้งร้าน · ดูสาเหตุได้ที่การ์ด "ใครเข้างานตอนนี้" ในหน้าตั้งค่าของแอป
- เทสอยู่ `kpi/test/` (`npm run check` + `npm test` ต้อง ALL PASSED ก่อนส่ง)
- แท็บเล็ตที่ยังชี้ URL เดิมของ repo JJ-KPI จะไม่ได้อัพเดตจาก repo นี้ — ต้องเปลี่ยนมาใช้ URL ใหม่

## ระบบเงินเดือน (JJ-Payroll)

- แอปคือ `jjmk-payroll.html` ที่ root เหมือนแอปอื่น (ย้ายมาจาก repo `thananant/JJ-Payroll` 2026-09-06) · ไฟล์ประกอบ (worker/tools/docs/logo) อยู่โฟลเดอร์ `jjmk-payroll/` — **อ่าน `jjmk-payroll/CLAUDE.md` ก่อนแก้ทุกครั้ง** (กฎเงินเดือนห้ามเปลี่ยนโดยไม่ถาม)
- แอปเปิดที่ https://thananant.github.io/JJ-PnL/jjmk-payroll.html — **ต้องล็อกอิน** (บัญชี `pnl_users` ชุดเดียวกับ P&L/Social) · ทุกการแก้ข้อมูลถูกจดใน `payroll_audit` ดูที่หน้า "🕘 ประวัติการทำรายการ"
- ฐานข้อมูล Supabase โปรเจกต์ `aikyxvluaiubdidqxwnd` (ตาราง employees/punches/adjustments/advances/... ดู `jjmk-payroll/docs/JJ-Payroll-Summary.md`)
- **งวดเงินเดือน = 26 เดือนก่อน → 25 เดือนนี้ (ไม่ตรงเดือนปฏิทิน)** — เวลาเอาค่าแรงเข้า PnL ต้องตกลงวิธีแบ่งก่อน · จุดเชื่อม Payroll↔PnL ดูข้อ 8 ใน `jjmk-payroll/docs/JJ-Payroll-Summary.md`
- `jjmk-payroll/worker.js` deploy ผ่าน Cloudflare dashboard (ไม่ใช่ GitHub) — เครื่องสแกน/รายงาน LINE ชี้ที่ Worker เดิม ไม่เกี่ยวกับการย้าย repo

## ระบบฟังเสียงลูกค้า (JJ Social)

- แอปคือ `jjmk-social.html` ที่ root · ไฟล์ติดตั้ง (SQL + `supabase/functions/social-brain.ts`, `social-webhook.ts` + `README-SOCIAL.md`) อยู่ branch `claude/social-listening-system-342kax`
- ตาราง Supabase ใช้ prefix `social_` · ล็อกอินใช้ `pnl_users` ร่วมกับ P&L · สาขาอ่านจาก `pnl_branches`
- Edge Functions deploy โดยวางโค้ดใน Dashboard (ชื่อฟังก์ชัน `social-brain` / `social-webhook` — ตัวหลังปิด Verify JWT)
- secrets ฝั่ง LINE ของระบบนี้คือ `LINE_CHANNEL_SECRET` / `LINE_CHANNEL_ACCESS_TOKEN` (OA หน้าร้าน) — **คนละตัวกับ `LINE_SECRET`/`LINE_TOKEN` ซึ่งเป็นของระบบอื่น ห้ามใช้ปน**
- **การเข้าสู่ระบบ (2026-09-21)**: เข้าจากหน้ารวมระบบ `index.html` → hub ออก "ใบผ่าน" `localStorage.jjsso_ticket` (อายุ 2 นาที ใช้ครั้งเดียว) → แอพตรวจกับ `pnl_users` จริงแล้วพาเข้าเลย · เปิดจากลิงก์ตรง = ต้องใส่รหัสผ่านทุกครั้ง · ใบอนุญาตของแท็บอยู่ใน `sessionStorage.jjsocial_auth` (ปิดเบราว์เซอร์ = หลุด) · สิทธิ์รายเมนูอ่านจาก `pnl_users.apps.social` (v=ดู, a/e/d=แก้ไข) — ไม่มีสิทธิ์เลย = เข้าไม่ได้ · รูปแบบนี้ใช้เป็นต้นแบบให้แอพอื่นได้
- **มือถือ (2026-09-21)**: ใช้ pull-to-refresh ตัวมาตรฐานเดียวกับทุกแอป (ลากลง → `location.reload()`) — **อย่าเขียนตัวที่สองซ้อน** · layout จอ ≤700px: หัวเป็น grid 2 แถว (ชื่อ+สถานะ / สาขา+ช่วงเวลา), KPI 2 คอลัมน์, ปุ่มและตัวกรองเต็มความกว้าง, แชทเห็นบทสนทนาโดยไม่ต้องเลื่อนไกล, เผื่อ safe-area ของ iPhone · รีเฟรชแล้วยังอยู่ในระบบเพราะใบอนุญาตอยู่ใน sessionStorage

## ระบบปฏิทินองค์กร (JJ Calendar)

- แอปคือ `jjmk-calendar.html` ที่ root (สร้าง 2026-09-10) — ปฏิทินนัดหมายภายในองค์กร: สร้างนัด ระบุผู้สร้าง/ผู้เข้าร่วม มุมมองเดือน+รายการนัด
- ตาราง Supabase ใช้ prefix `cal_` (`cal_events`/`cal_attendees`/`cal_settings`) · ล็อกอินใช้ `pnl_users` ร่วมกับ P&L · สาขาอ่านจาก `pnl_branches` (+ JJCK/OFFICE เพิ่มในแอป)
- SQL ติดตั้งคือ `jjmk-calendar.sql` ที่ root ของ branch `sql`
- เพิ่มนัดลงมือถือได้ 2 ทาง: ดาวน์โหลด `.ics` ต่อนัด (มี VALARM เตือนตาม `reminder_min`) / subscribe ฟีดทั้งปฏิทินผ่าน Edge Function **`cal-feed`** (deploy ผ่าน Dashboard · **ปิด Verify JWT** · URL ต้องมี `?token=` ตรงกับ `cal_settings` id=`feed` · `&user=` = กรองเฉพาะนัดของคนนั้น)
- นัดทั้งวันเก็บ `start_at`/`end_at` เป็น 00:00 เวลาไทยของวันแรก/วันสุดท้าย (นับรวม) — ฝั่ง ICS แปลง DTEND เป็น exclusive (+1 วัน) ให้แล้ว
- ยกเลิกนัด = `cancelled=true` (soft delete) → หายจากแอปและฟีดมือถือตอนรีเฟรช
- **แก้ปัญหาฟีดไม่ทำงาน** (ปุ่ม 🔎 ทดสอบในแท็บ "เชื่อมมือถือ" แยกอาการให้แล้ว): `401` = Supabase ปัดตกตั้งแต่ด่านหน้า → ยังไม่ได้ deploy `cal-feed` **หรือยังไม่ได้ปิด Verify JWT** (แอปยิงซ้ำแบบแนบ key เพื่อแยกสองกรณี: ได้ 404 = ยังไม่ deploy · ได้ 200/403 = ยังไม่ปิด JWT) · `403` = token ไม่ตรงกับ `cal_settings`
- **โค้ด `cal-feed` อยู่ในแอปแล้ว** — ฝังเป็น `<script type="text/plain" id="feedCode">` ในหัวไฟล์ + ปุ่ม 📋 คัดลอกในแท็บ "เชื่อมมือถือ" (เห็นเฉพาะ `isBoss()`) · **แก้โค้ดฟีดต้องแก้ 2 ที่ให้ตรงกัน**: บล็อกนี้ กับ `cal-feed.ts` ที่ branch `sql`
- โค้ดฟีดเป็น **TypeScript วางที่ Edge Functions เท่านั้น** — ถ้าวางใน SQL Editor จะขึ้น `syntax error at or near "//"` (เคยพลาดมาแล้ว 2026-09-21 · ขั้นตอนติดตั้งจึงอยู่ในแอปแล้ว)
- `fold()` ในโค้ดฟีด **ต้องพับบรรทัดตามจำนวนไบต์ (74) และเดินด้วย `for...of`** — ของเดิมหั่นทีละ 73 ตัวอักษร JS ทำให้อีโมจิถูกหั่นกลางตัว (surrogate เดี่ยว) และบรรทัดยาว 200+ ไบต์ เกินมาตรฐาน RFC 5545 (แก้แล้ว 2026-09-21)
- ⚠️ **ห้ามเขียนไฟล์โค้ดที่มีแบ็กสแลชด้วย bash heredoc** — สภาพแวดล้อมนี้กลืนแบ็กสแลชในลำดับ `\\;` (พิสูจน์แล้ว 2026-09-21: `E\\;F` กลายเป็น `E\;F`) ทำให้ `esc()` ของฟีดไม่ escape `;` ให้ใช้ python เขียนไฟล์แทน และตรวจไบต์จริงด้วย `od -c` ทุกครั้ง
- `500` จากฟีด = ผ่าน Verify JWT แล้วแต่โค้ดพัง — **ฟังก์ชันตอบสาเหตุกลับมาเป็นข้อความ** และปุ่มทดสอบในแอปเอามาแสดงให้เลย · โปรเจกต์นี้ใช้คีย์แบบใหม่ (`sb_publishable_...`) คีย์ legacy `SUPABASE_SERVICE_ROLE_KEY` อาจใช้ไม่ได้ → `sbGet()` จึง**ลอง service role ก่อนแล้วสลับไป `SUPABASE_ANON_KEY` เอง** (ตาราง `cal_*` เปิด select ให้ anon อยู่แล้ว)
- **ปุ่มแตะครั้งเดียว (2026-09-21)**: แท็บ "เชื่อมมือถือ" มีปุ่มใหญ่ 2 ปุ่ม — iPhone ใช้ลิงก์ `webcal:` (เปิดแอปปฏิทินให้เลย) · Android ส่งไป `calendar.google.com/calendar/render?cid=<webcal url>` · เรียงปุ่มตามเครื่องที่เปิด และแอปตรวจฟีดให้เองตั้งแต่เปิดหน้า (ฟีดไม่พร้อม = ปุ่มจางลงพร้อมบอกเหตุผล)
- ~~iPhone ห้ามใช้ `webcal:`~~ **ข้อมูลเดิมผิด** — ที่ iPhone ขึ้น "การตรวจสอบไม่สำเร็จ" ตอน 2026-09-21 เกิดจากฟีดตอบ 401 (ยังไม่ปิด Verify JWT) ไม่ใช่เพราะ scheme · `webcal:` เป็นวิธีมาตรฐานที่ Google/Outlook ใช้กันอยู่แล้ว · ถ้าแตะแล้วไม่ขึ้นจริง ๆ ค่อยใช้วิธีคัดลอก URL `https://` ไปวางใน การตั้งค่า → แอปปฏิทิน → เพิ่มปฏิทินที่ติดตาม (อยู่ในกล่อง "ปุ่มไม่ทำงาน?" ในแอป)

- **เรียลไทม์ + ตัวกรอง (2026-09-21)**: แอปต่อ Supabase Realtime (`supabase-js` จาก CDN · channel `jjcal-live` ฟัง `cal_events`/`cal_attendees`) → มีคนเพิ่มนัด คนอื่นเห็นทันที · **ต้องรัน `jjmk-calendar-realtime.sql` ครั้งเดียว** (เพิ่มตารางเข้า publication `supabase_realtime`) ไม่งั้นจะตกไปใช้โพลสำรองทุก 90 วินาที · จุด 🟢/⚪ ข้างชื่อผู้ใช้บอกสถานะการเชื่อมต่อสด · ไม่รีเฟรชทับตอนเปิด modal ค้างอยู่ (กันฟอร์มที่กรอกค้างหาย)
- ตัวกรองในหน้าปฏิทิน/รายการนัด: ค้นหา (หัวข้อ·สถานที่·รายละเอียด·ชื่อสาขา·ชื่อผู้เข้าร่วม) + เลือกดูปฏิทินของคนใดคนหนึ่ง + ปุ่มล้างตัวกรอง · เก็บใน `sessionStorage.jjcal_filter` (รีเฟรชแล้วยังอยู่ ปิดแท็บแล้วหาย) · กล่องเลือกผู้เข้าร่วมกรองตามสังกัด (`pnl_users.unit`) ได้ และ "เลือกทั้งหมดที่เห็น" จะเลือกเฉพาะที่ผ่านตัวกรอง

## ระบบล็อกอินกลาง (SSO) — หน้าศูนย์รวมแอป (เจ้าของสั่ง 2026-09-18)

- **พนักงานล็อกอินที่หน้าแรก (`index.html`) ที่เดียว** แล้วเห็นเฉพาะแอปที่ตัวเองได้รับสิทธิ์ (อ่านจาก `pnl_users.apps` ที่ admin ตั้งในหน้า JJ Access)
- ล็อกอินสำเร็จ → หน้าแรก**แจกใบผ่านให้ทุกแอปที่ใช้บัญชี `pnl_users`** ผ่าน localStorage (ฟังก์ชัน `spreadSession`) จึงเข้าแอปต่อได้เลยไม่ต้องกรอกซ้ำ:
  `jjpnl_auth` (P&L · Owner · Access) · `jjpay_sess` (Payroll) · `jjsocial_sess` · `jjcal_sess` · `jjmk_inv_sess`
  — **เพิ่มแอปใหม่ที่ใช้ `pnl_users` ต้องเพิ่มคีย์ใน `spreadSession` และ `clearSession` ด้วย** ไม่งั้นต้องล็อกอินซ้ำ
- **"ติ๊กแอปไหนให้ ก็เห็นแอปนั้น"** — ไอคอนบนหน้าแรกตรงกับที่ตั้งใน JJ Access เป๊ะ ไม่มีข้อยกเว้น (เจ้าของสั่ง 2026-09-21 — เดิมมีรายการ `ALWAYS` ให้ KPI/นับสต๊อก/ครัวกลาง/ซ่อมบำรุงแสดงเสมอ **เอาออกแล้ว** เพราะทำให้เห็นแอปเกินสิทธิ์ที่ให้)
- **ทุกแอปใช้บัญชีกลาง `pnl_users` ชุดเดียวกันหมดแล้ว (2026-09-24)** — นับสต๊อก (`sc_users`) และครัวกลาง (Supabase Auth + `app_users`) เป็น 2 ตัวสุดท้ายที่ย้ายมา · ติ๊กที่ JJ Access จึงคุมทั้ง "เห็นไอคอนไหม" และ "เข้าแอปนั้นได้ไหม"
  - **นับสต๊อก (`jjmk-stockcheck.html`)**: เดิม hash ใช้ salt `JJSC` คนละตัวกับบัญชีกลาง (`JJPNL`) → เอา user/pass กลางไปกรอกยังไงก็ไม่ผ่าน · ตอนนี้ใช้ `pwHash(u,p)=sha256(u+'|'+p+'|JJPNL')` เหมือนกันแล้ว · เซสชันประจำแท็บ `sessionStorage.jjsc_sess` · `unit` ของบัญชีกำหนดสาขาที่เห็น (JJLP→ลาดพร้าว · JJRD→รัชดา · ALL/OFFICE/JJCK→ทุกสาขา) · มีสิทธิ์หน้า `cfg`/`set`/`items`/`sched` = เป็นผู้ดูแลในแอป · หน้า "จัดการผู้ใช้" เดิมเอาออกแล้ว ชี้ไป 🔑 JJ Access แทน
  - **ครัวกลาง (`jjmk-kitchen.html`)**: เลิกใช้ Supabase Auth (`signInWithPassword` + อีเมลปลอม `@jjmk.local`) และตาราง `app_users` · เซสชันประจำแท็บ `sessionStorage.jjck_sess` · "จำไว้ในเครื่องนี้ 12 ชม." = `jjck_keep` · role แปลงเป็นของเดิมในแอป: admin/owner→`owner` · manager หรือมีสิทธิ์หน้า `set`/`acc`→`manager` · ที่เหลือ→`warehouse`
  - **ต้องรัน `jjmk_kitchen_central.sql` (branch `sql`) ครั้งเดียว** ไม่งั้นครัวกลางบันทึกอะไรไม่ได้ — RLS เดิมของตาราง `ck_*` บังคับ `authenticated` + เช็ค `app_users.auth_uid` ซึ่งบัญชีกลาง (ยิงผ่าน anon key) ทำไม่ได้ · ไฟล์เปิด `ck_*` + `ck_add_qty` + bucket `product-images` ให้ role `anon` (แพตเทิร์นเดียวกับ `jjmk_maint_access.sql`)
  - ปุ่ม "ดูแอปทั้งหมด" ที่หน้าล็อกอินยังอยู่เป็นทางออกฉุกเฉินสำหรับคนที่ยังไม่มีบัญชีกลาง (flag `jjhub_skip`) — **อย่าตัดออก**
  - ซ่อมบำรุงยังรับ `app_users` เป็นตัวสำรองในหน้าล็อกอิน (บัญชีเดิมของนับสต๊อก/ครัวกลาง) — ตั้งใจให้เป็นแบบนั้น
- **ซ่อมบำรุง (`jjmk-maint.html`) ต่อล็อกอินกลางแล้ว (2026-09-21)** — ใช้ใบผ่าน `jjsso_ticket` มาตรฐานเดียวกับ JJ Social:
  - ใบผ่านจากหน้าแรกอายุ 2 นาที ใช้ครั้งเดียว → แอปเอา `u`+`h` ไปตรวจกับ `pnl_users` จริงอีกชั้นก่อนพาเข้า
  - **เปิด URL ของแอปตรง ๆ = ต้องกรอกรหัสใหม่เสมอ** (เจ้าของสั่ง: กลัวคนได้ลิงก์ไปแล้วเข้าได้) · เซสชันอยู่ใน `sessionStorage` (`jjmt_sess`) → **ดึงหน้าลง/รีเฟรชบนมือถือยังอยู่หน้าเดิม** ปิดแท็บแล้วต้องล็อกอินใหม่
  - ติ๊ก "จำไว้ในเครื่องนี้ 12 ชม." → `jjmt_keep` (username+pass_hash · ตรวจกับ `pnl_users` ใหม่ทุกครั้งที่เปิด) สำหรับมือถือส่วนตัวของพนักงาน
  - หน้าล็อกอินรับได้ทั้ง **`pnl_users`** (บัญชีกลาง — ลองก่อน) และ **`app_users`** (บัญชีเดิมของนับสต๊อก/ครัวกลาง — ลองต่อถ้าไม่เจอ)
  - บัญชีกลางไม่มี session ของ Supabase Auth → ต้องรัน `jjmk_maint_access.sql` (branch `sql`) เปิดสิทธิ์เขียน `maint_*` + อัปโหลด `maint-photos` ให้ role `anon` เหมือนแอปกลางตัวอื่น
  - หน้าจอในระบบสิทธิ์ (JJ Access): `sched` ตารางบำรุงรักษา · `log` บันทึกงาน+รูปถ่าย · `asset` นับอุปกรณ์+สถิติ · `tasks` ตั้งค่างาน/อุปกรณ์
- **JJ KPI (`jjmk-kpi.html`) ต่อล็อกอินกลางแล้ว (2026-09-21)** — ใบผ่าน `jjsso_ticket` มาตรฐานเดียวกัน (แอปสุดท้ายที่เคยเปิดโล่งให้คนมีลิงก์เข้าดูได้):
  - แดชบอร์ดต้องล็อกอิน · เปิดลิงก์ตรง = กรอกรหัสใหม่เสมอ · เซสชันอยู่ `sessionStorage` (`kpi_auth`) → **ลากหน้าลง/รีเฟรชบนมือถือยังอยู่หน้าเดิม** ปิดแท็บแล้วต้องล็อกอินใหม่
  - **จอ kiosk ของลูกค้าหน้าร้าน (`?kiosk=`) ไม่ต้องล็อกอิน** — ห้ามใส่ประตูตรงนั้น แท็บเล็ตจะใช้ไม่ได้
  - หน้าจอในระบบสิทธิ์: `dash` แดชบอร์ดเจ้าของ · `kiosk` หน้าจอลูกค้า · เทสครอบคลุมใน `kpi/test` กลุ่ม [9]
- **กันล็อกคนใช้งานโดยไม่ตั้งใจ** (สำคัญ — อย่าตัดออก):
  - `apps` ว่าง (ยังไม่เคยตั้งสิทธิ์) = แสดงทุกแอปตามเดิม ไม่ซ่อนอะไร
  - เน็ตล่ม/เช็คสิทธิ์ไม่ได้ = แสดงทุกแอปไว้ก่อน
  - ปุ่ม "ดูแอปทั้งหมด" ในหน้าล็อกอิน สำหรับคนที่ใช้บัญชีของระบบนับสต๊อก/ครัวกลาง (เก็บ flag `jjhub_skip`)
- ไอคอน 🔑 JJ Access เห็นเฉพาะ `role='admin'` (ยังไม่ล็อกอิน+ไม่เคยล็อกอินค้าง = ซ่อน)
- ออกจากระบบที่หน้าแรก = ล้างใบผ่านทุกคีย์พร้อมกัน (`clearSession`)

### ด่านกันลิงก์หลุด — JJ Gate (เจ้าของสั่ง 2026-09-21)

**กติกา: เข้าจากหน้าศูนย์รวมแอพ = ใช้ได้เลยตามสิทธิ์ · เข้าด้วย URL ตรง = ต้องใส่รหัสผ่านใหม่ทุกครั้ง**
(กันกรณีลิงก์ถูกส่งต่อ แล้วเครื่องนั้นมี session ค้างอยู่ → เดิมเปิดเข้าได้เลย)

- โค้ดด่านคือบล็อก `window.JJGate` (+ `<style>` คู่กัน) ฝังไว้ก่อน `</head>` ของ **7 ไฟล์**:
  `index.html` · `jjmk-pnl.html` · `jjmk-owner.html` · `jjmk-admin.html` · `jjmk-payroll.html` ·
  `jjmk-calendar.html` · `jjmk-invoice.html`
  — **บล็อกเหมือนกันทุกไฟล์ ต่างแค่ `var APP=` บรรทัดแรก · แก้ที่ไหนต้องแก้ให้ครบทุกไฟล์**
- **JJ Social + ซ่อมบำรุง ไม่ได้ฝังบล็อกนี้** — สองแอพนั้นมีระบบของตัวเองที่ทำเรื่องเดียวกัน
  (ใบผ่าน `jjsso_ticket` + session ประจำแท็บใน `sessionStorage`) ทำงานร่วมกันได้ ไม่ต้องรวมเป็นอันเดียว:
  หน้าศูนย์รวมแอพออกใบผ่าน **ทั้งสองแบบ** ตอนคลิก (`JJGate.issue` + `issueTicket`)
  และ `JJGate.ok()` รับใบผ่านแบบ `jjsso_ticket` ได้ด้วย · **ใช้ใบผ่านแล้วลบทิ้งทั้งคู่** (ฟังก์ชัน `burn()`)
  ไม่งั้นใบที่เหลือค้างจะถูกเอาไปเปิดแท็บใหม่เข้าได้อีก
- ผ่านด่านได้ 3 ทาง (ฟังก์ชัน `JJGate.ok(username)`):
  1. **ใบผ่านประจำแท็บ** (`sessionStorage['jjgate_<app>']`) — ดึงหน้าลง/กดรีเฟรชแล้วยังอยู่ · แท็บใหม่ไม่มี · ไม่ขยับเกิน 30 นาทีแล้วรีเฟรช = ยืนยันใหม่
  2. **ใบผ่านครั้งเดียวจากหน้าศูนย์รวมแอพ** (`localStorage['jjhub_pass']`) — หน้าแรกออกให้ตอนกดไอคอน อายุ 2 นาที ผูกกับชื่อผู้ใช้+ชื่อแอพ ใช้แล้วลบทิ้ง
  3. **เปิดจากไอคอนที่ติดตั้งบนหน้าจอมือถือ (PWA standalone)** — ถือว่าเป็นเครื่องของเจ้าตัว
     (ลิงก์ที่ส่งต่อทางแชทเปิดในเบราว์เซอร์ธรรมดา ไม่เข้าเงื่อนไขนี้)
- ไม่ผ่าน → แอพแสดงหน้าล็อกอินของตัวเอง + ข้อความ "🔒 เปิดจากลิงก์ตรง" + เติมชื่อผู้ใช้ให้ (`JJGate.note(user,'liUser','liPass')`)
  · ล็อกอินสำเร็จทุกแอพต้องเรียก **`JJGate.grant(username)`** ไม่งั้นรีเฟรชแล้วจะถามรหัสซ้ำ
- หน้าศูนย์รวมแอพออกใบผ่านด้วย `JJGate.issue(app, ME.username)` ตอนคลิกไอคอน — **ยังไม่ล็อกอิน = ไม่ออกให้** (แอพปลายทางจะถามรหัสเอง) · `logout()` ล้าง `jjhub_pass` ด้วย
- **เพิ่มแอพใหม่ที่ใช้ `pnl_users`** ต้องทำ 3 อย่าง: ฝังบล็อก JJ Gate (ตั้ง `APP` ให้ตรง `data-app`) · เช็ค `JJGate.ok()` ก่อนยอมรับ session ที่เก็บไว้ · เรียก `JJGate.grant()` ตอนล็อกอินสำเร็จ
- ช่องล็อกอินบนมือถือถูกบังคับเป็น 16px (`#lgU,#lgP,#liUser,#liPass`) กัน iPhone ซูมหน้าจอเองตอนกดพิมพ์
- เช็คบัญชีกับฐานข้อมูลไม่ได้ (เน็ตล่ม/ตาราง error) = **ไม่ปล่อยเข้า** แล้ว — เดิม P&L/Owner หลุดเป็น `legacy` ที่เป็น admin (ยกเว้นตาราง `pnl_users` ไม่มีจริง = HTTP 404 ถึงจะเข้าโหมดเดิม)

### ระดับตำแหน่ง + สังกัด (เจ้าของสั่ง 2026-09-18)

บัญชีใน `pnl_users` มี 2 มิติ — ตั้งที่หน้า JJ Access → ผู้ใช้ & สิทธิ์ → ✏️ (SQL: `jjmk-access-unit.sql`)

| `role` (ระดับ) | ได้อะไร |
|---|---|
| `admin` 🛡️ ผู้ดูแลระบบ | ทุกสิทธิ์ทุกแอป + เข้าหน้า JJ Access จัดการผู้ใช้ |
| `owner` 👑 เจ้าของ | เหมือน admin (ทุกแอป + จัดการผู้ใช้) |
| `manager` 📋 ผู้จัดการ | ตามสิทธิ์รายแอปที่ตั้งให้ · เข้า JJ Access ไม่ได้ |
| `staff` 👤 พนักงาน | ตามสิทธิ์รายแอปที่ตั้งให้ · เข้า JJ Access ไม่ได้ |

| `unit` (ประจำที่) | |
|---|---|
| `ALL` ส่วนกลาง · `OFFICE` ออฟฟิศ · `JJCK` ครัวกลาง · `JJLP` สาขาลาดพร้าว · `JJRD` สาขารัชดา | |

- ฐานข้อมูลมี CHECK constraint คุมค่าทั้ง `role` และ `unit` — เพิ่มค่าใหม่ต้องแก้ constraint ด้วย
- (2026-09-21 แก้แล้ว: `jjmk-owner.html` เคยเช็ค `role==='admin'` ตรง ๆ ทำให้บัญชีระดับ **เจ้าของ** เข้าหน้าเจ้าของไม่ได้ · `jjmk-social.html` `isAdmin()` และ `jjmk-calendar.html` `canEdit()` ก็แก้ให้นับ `owner` ด้วยแล้ว)
- โค้ดเช็คสิทธิ์สูงสุดใช้ `isBoss(u)` = `role` เป็น admin หรือ owner — **อย่าเช็ค `role==='admin'` ตรง ๆ** ไม่งั้นเจ้าของจะหลุดสิทธิ์
- `unit` ยังเป็นข้อมูลกำกับ (แสดงในหน้าจัดการผู้ใช้ + แถบผู้ใช้) — ถ้าจะใช้กรองข้อมูลรายสาขาในแอปไหน ค่อยต่อยอดจากคอลัมน์นี้


## กติกา UX ทุกแอป — รีเฟรชแล้วต้องอยู่หน้าเดิม (เจ้าของสั่ง 2026-09-18)

- **ลากลง (pull-to-refresh) หรือกดรีเฟรช = ต้องกลับมาที่หน้า/แท็บเดิมที่เปิดค้างไว้** ห้ามเด้งกลับหน้าแรก
- วิธีทำ: ตอนสลับแท็บให้เก็บลง `localStorage` (คีย์ `jj<ระบบ>_tab` เช่น `jjcal_tab`, `jjsocial_tab`)
  แล้วตอนเปิดแอปอ่านค่ากลับมาใช้เป็นแท็บเริ่มต้น (ต้อง validate ว่าเป็นแท็บที่มีอยู่จริง กันค่าเสียจาก version เก่า)
- หน้าที่ไม่มีแท็บ ให้จำ "มุมมองที่ดูค้างไว้" แทน เช่น JJ Owner จำปี+สาขา (`jjowner_view`)
- เขียนแอปใหม่ต้องมีตั้งแต่แรก · สถานะปัจจุบัน (2026-09-18) ทำครบทุกแอปแล้ว:
  P&L `jjpnl_view` · Kitchen `jjck_tab` · นับสต๊อก `jjsc_tab` · Payroll (hash routing) ·
  Social `jjsocial_tab` · Calendar `jjcal_tab` · KPI `kpi_tab` · Invoice `jjinv_tab` ·
  Access `jjacc_tab` · Owner `jjowner_view` · ซ่อมบำรุง `jjmt_tab` (แท็บตารางบำรุงรักษา/นับอุปกรณ์)
- **ทุกแอปใส่ meta no-cache ใน `<head>`** (`Cache-Control`/`Pragma`/`Expires` — ทำครบทุกไฟล์แล้ว 2026-09-21) เพราะดึงหน้าลง (pull-to-refresh) บนมือถือแล้วยังได้ไฟล์เก่าค้าง · **เขียนแอปใหม่ต้องใส่ด้วยทุกครั้ง**
- หน้าศูนย์รวมแอป (`index.html`) และ Calendar มีปุ่ม 🔄 โหลดใหม่ (ข้ามแคชด้วย `?v=`) เพิ่มอีกชั้น — `hardReload()`
- **ลากหน้าลง = รีเฟรช (pull-to-refresh) — มีครบทุกแอปแล้ว** (เจ้าของสั่ง 2026-09-21):
  แอปที่ "เพิ่มไปยังหน้าจอโฮม" (PWA standalone) ไม่มีท่ารีเฟรชของเบราว์เซอร์ จึงต้องทำเอง —
  สคริปต์อยู่ท้าย `<script>` ของทุกไฟล์ (ค้นด้วยคำว่า `ลากหน้าลง = รีเฟรช`)
  - แตะที่ยอดสุดของหน้าแล้วลากลงเกิน 70px → ปล่อยนิ้ว = `location.reload()` · มีแถบบอก "ลากลงเพื่อรีเฟรช / ปล่อยเพื่อรีเฟรช"
  - ไม่ทำงานเมื่อ: ไม่ใช่จอสัมผัส · หน้าเลื่อนลงมาแล้ว · ลากอยู่ในกล่องที่เลื่อนได้เอง (ตาราง/ลิสต์)
  - ⚠️ **โหมด kiosk ของ KPI ปิดไว้** (`body.kiosk` หรือ URL มี `?kiosk=`) — จอลูกค้าใช้ `overflow:hidden` อยู่ยอดสุดเสมอ ถ้ารีเฟรชคะแนนที่ลูกค้ากดค้างไว้จะหาย **ห้ามตัดเงื่อนไขนี้ออก**
  - แอปใหม่ต้องก๊อปสคริปต์นี้ไปด้วย
  - ⚠️ ลากลงแล้ว `location.reload()` = **ฟอร์มที่กรอกค้างหายทั้งใบ** — หน้าที่มีฟอร์มยาวต้องเก็บร่างไว้ใน
    `localStorage` แล้วกู้คืนตอนเปิดแอป (ทำแล้วที่ใบกำกับภาษี: `jjinv_draft` อายุ 12 ชม.
    `saveDraftSoon()`/`restoreDraft()` · ล้างทิ้งทันทีที่ออกบิลสำเร็จหรือกดล้างฟอร์ม)

### กติกาหน้าจอมือถือ ทุกแอป (เจ้าของสั่ง 2026-09-21)

- ช่องกรอกบนจอ ≤700px ต้อง `font-size:16px` ขึ้นไป ไม่งั้น iPhone ซูมหน้าเองตอนกดพิมพ์
- ปุ่มสูงอย่างน้อย 44px (`.btn-sm` 40px) · แถบแท็บ `position:sticky;top:0` จะได้สลับแท็บโดยไม่ต้องเลื่อนขึ้น
- ตารางที่มีหลายคอลัมน์ให้แปลงเป็นการ์ดต่อแถวที่จอ ≤560px (ซ่อน `thead` + `display:grid` ที่ `tr` +
  ป้ายชื่อคอลัมน์ด้วย `td::before`) — ตารางแคบ ๆ บนมือถือกรอกไม่ได้จริง
- ข้อความยาวบนหัวจอ (ชื่อบริษัท/ชิปผู้ใช้) ห่อส่วนขยายด้วย `<span class="chipmore">` แล้วซ่อนที่จอ ≤700px
- ใส่ `overscroll-behavior-y:contain` ที่ `html,body` กันรีเฟรชของเบราว์เซอร์ซ้อนกับแถบลากลงของแอป
- มือถือเปิด PDF ด้วย `window.open(blob)` มักได้หน้าขาว → ให้ดาวน์โหลดไฟล์แทน (ตัวแปร `IS_TOUCH`)
- ตรวจทุกแท็บว่า `scrollWidth <= innerWidth` (หน้าต้องไม่เลื่อนแนวนอน) ก่อนส่งงาน

## กติกาการส่งงาน — ใช้กับ **ทุกระบบ** ใน repo นี้ (P&L · Owner · KPI · Payroll · นับสต๊อก · ซ่อมบำรุง · Kitchen · Social · Invoice · Calendar · Access)

- ไฟล์ที่ต้องรัน/วางบน **Supabase** (SQL, โค้ด Edge Functions) → **แปะเนื้อหาลงในแชทเป็นบล็อกโค้ดเลย** (เจ้าของสั่ง 2026-09-17 หลังลองมาแล้ว 2 แบบ):
  - เจ้าของกดคัดลอกจากบล็อกโค้ดในแชทไปวางใน Supabase SQL Editor ได้ทันที **ไม่ต้องดาวน์โหลด ไม่ต้องกดเปิดไฟล์**
  - ❌ **ห้ามส่งเป็นการ์ดไฟล์ (`SendUserFile`)** — กดแล้วเด้งหน้าต่างเซฟลงเครื่อง เสียเวลา
  - ยังเขียนไฟล์ลงโฟลเดอร์ `sql/` ไว้ด้วย (อยู่ใน `.gitignore`) แล้วบอก path สั้น ๆ ไว้ท้ายข้อความ เผื่อเจ้าของอยากเปิดดูในแผงขวา — แต่**เนื้อหาต้องอยู่ในแชทเสมอ**
  - **ยังต้อง push เก็บที่ branch `sql` ทุกครั้ง** ตามกติกา repo ข้อแรก (ของ payroll ไปโฟลเดอร์ `jjmk-payroll/` ของ branch นั้น)
  - ถ้าไฟล์ยาวมากจนแปะทั้งก้อนไม่ไหว ให้แบ่งเป็นส่วน ๆ แล้วแปะต่อกันในข้อความเดียว — ห้ามให้เจ้าของไปตามหาเอง
  - โค้ดยาว ๆ ที่เจ้าของต้องเอาไปวางที่อื่น (Apps Script, Cloudflare Worker) ใช้หลักเดียวกัน — แปะในแชท หรือทำปุ่มคัดลอกในแอป
- ไฟล์ที่ขึ้น **GitHub** (แอป HTML ฯลฯ) → **push ขึ้น main ให้เองเลย** ไม่ต้องส่งในแชท ไม่ต้องถามยืนยัน
