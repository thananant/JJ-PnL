...
setTimeout(async()=>{
  ...
  // 4) แถบสรุปในการ์ดหัว
  const pc=[...d.querySelectorAll('#view-recon .pcell')].map(x=>x.textContent);
  const hasBox=(l,v)=>pc.some(x=>x.includes(l)&&x.includes(v));
  out.push('กล่อง: เงินโอนตรง 1 วัน จาก 2 วันที่เทียบได้: '+hasBox('เงินโอนตรง','1 วัน'));
  out.push('กล่อง: คีย์ขาด ฿1,500.00 (1 วัน): '+hasBox('คีย์ขาด/ลืมคีย์','฿1,500.00'));
  out.push('กล่อง: สุทธิ +฿1,500.00 เงินเข้าจริงมากกว่าที่คีย์: '+hasBox('สุทธิ +/− จากยอดที่คีย์','+฿1,500.00'));
  out.push('กล่อง: รอเงินเข้า ฿39,972.74: '+hasBox('รอเงินเข้าบัญชี','฿39,972.74'));
  out.push('กล่อง: ปิดกะ 1 กะไม่ลงตัว ขาดรวม ฿1,000.00: '+(hasBox('ปิดกะ','1 กะไม่ลงตัว')&&hasBox('ปิดกะ','ขาดรวม ฿1,000.00')));
