...
- กติกาเจ้าของ (2026-09-17 — แทนที่กติกา 2026-09-08 เรื่องวิธีส่ง SQL): SQL ที่ต้องรันใน Supabase → **เขียนไฟล์ลง `sql/` ในเครื่อง session แล้วบอก path ให้เจ้าของกดเปิดอ่านด้านขวา** (เช่น `sql/jjmk-access.sql`) — เจ้าของจะได้ copy ไปวางใน SQL Editor ได้เลยโดยไม่ต้องดาวน์โหลด · **ห้ามใช้ SendUserFile กับไฟล์ SQL** (กลายเป็นการ์ดให้โหลด ใช้ยาก) · โฟลเดอร์ `sql/` อยู่ใน `.gitignore` แล้ว — ยัง push เก็บที่ branch `sql` ตามกติกาข้อแรกด้วยเสมอ · งานที่ขึ้น GitHub ให้ push/merge เองเลย ไม่ต้องส่งไฟล์มาในแชท
...
- ไฟล์ที่ต้องรัน/วางบน **Supabase** (SQL, โค้ด Edge Functions) → **เขียนลงโฟลเดอร์ `sql/` ในเครื่อง session แล้วบอก path ให้กดเปิด** (อ่าน/copy ได้จากแผงขวาทันที ไม่ต้องดาวน์โหลด) — ห้ามส่งเป็นไฟล์แนบ
