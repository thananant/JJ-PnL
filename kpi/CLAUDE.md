...
· **กราฟการกระจายคะแนน (`histHTML`) ซ่อนแท่ง 😐 อัตโนมัติเมื่อช่วงที่ดูไม่มีคะแนน 3 เลย** (2026-09-21) — ข้อมูลใหม่จะเห็น 4 แท่งตรงกับจอลูกค้า ช่วงที่มีข้อมูลเก่ายังเห็น 5 แท่ง
...
หน้าตั้งค่า (ปรับใหม่ 2026-09-21 — เจ้าของสั่ง "คุมที่ตำแหน่ง ไม่ใช่รายคน"):
- **การ์ดหลัก "ตำแหน่งที่ให้ลูกค้าชม"** — ติ๊กเปิด/ปิดทั้งตำแหน่ง (เก็บ `kpi_settings.kiosk_hide_pos`) แสดงจำนวนคน + ตัวอย่างชื่อ
- **การ์ด "รายชื่อพนักงาน"** = อ่านอย่างเดียว จัดกลุ่มตามตำแหน่ง อยู่ในกล่องพับ (`<details>`) — เหลือแค่ติ๊กซ่อนรายคนเป็นกรณียกเว้น (`saveStaff` ส่งแค่ `{id, active}`)
  · **ไม่มีปุ่ม "+ เพิ่มพนักงาน" แล้ว** — แถวกรอกมือไม่ผูก `employee_id` จึงไม่มีวันขึ้นจอลูกค้า (กฎ on-duty) เพิ่ม/แก้ชื่อต้องทำที่แอป Payroll
- **การ์ด "ใครเข้างานตอนนี้"** — บอกว่าตอนนี้ระบบเห็นใครเข้างาน/ไม่เห็นเพราะอะไร (ยังไม่รัน SQL · ไม่มีใครสแกน · ยังไม่ผูก payroll)
...
### แท็บรายวัน — การ์ด "รายการล่าสุด" (2026-09-21)
แสดงล่าสุด `RECENT_STEP` = 30 แถว (เดิมตัน 15 · เจ้าของบอกว่าดูได้น้อยไป) · หัวการ์ดบอกจำนวนครั้งทั้งหมดของวันนั้น
· ปุ่ม "ดูทั้งหมด N ครั้ง" กางให้ครบ (เก็บสถานะใน `S.recentAll` · วาดใหม่เฉพาะการ์ด ไม่โหลดข้อมูลซ้ำ — `recentRows`)
· ตารางเลื่อนในกล่อง `.recent-scroll` (สูงสุด 70vh) หัวตาราง sticky
...
→ คำถามสุดท้าย "ชมพนักงาน 💖 อยากชมพนักงานคนไหนเป็นพิเศษไหมคะ?" — เลือกพนักงาน 1 คนของสาขานั้น
  ...
  **จัดกลุ่มตามตำแหน่ง** (กลุ่มใหญ่ขึ้นก่อน) · **โชว์เฉพาะคนกำลังเข้างาน** (`kpi_on_duty` — นับสแกนวันทำการคี่=อยู่
  ...
  · **ลิสต์ว่าง/RPC ล้ม → ข้ามคำถามชมพนักงานไปเลย (คืนลิสต์ว่าง)** — ห้าม fallback เป็น "โชว์ทั้งหมด" เด็ดขาด
    (เจ้าของลองใช้จริง 2026-09-21 แล้วชื่อขึ้นทั้งร้าน ลูกค้ากดผิดคน ข้อมูลเพี้ยน) · **แถวที่ไม่ผูก `employee_id` ก็ไม่โชว์**
    เพราะเช็คไม่ได้ว่าอยู่ร้านไหม · หน้าตั้งค่ามีการ์ด "ใครเข้างานตอนนี้" บอกสาเหตุ (ยังไม่รัน SQL / ไม่มีใครสแกน / ยังไม่ผูก payroll)
