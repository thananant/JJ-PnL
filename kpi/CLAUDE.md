...
- **วิธีส่ง SQL ให้เจ้าของ (กติกา 2026-09-17 — ใช้กับทุกระบบใน repo นี้)**: เขียนไฟล์ลงโฟลเดอร์
  `sql/` ในเครื่อง session แล้วส่งให้เจ้าของ **ทั้ง 2 ทาง**:
  (1) **การ์ดไฟล์ในแชท** (`SendUserFile`) ให้เห็นชัดว่ามีไฟล์ต้องเอาไปรัน
  (2) **บอก path** เช่น `sql/jjmk-kpi.sql` ให้กดเปิดอ่าน/copy จากแผงขวาได้ทันทีโดยไม่ต้องดาวน์โหลด
  · `sql/` อยู่ใน `.gitignore` แล้ว และยังต้อง push เก็บที่ branch `sql` ตามกติกาเดิมทุกครั้ง
- 2 โหมดจาก URL เดียว
